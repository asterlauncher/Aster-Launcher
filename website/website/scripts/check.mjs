...
try {
  ...
  latest.assets = [{ name: "Aster.Launcher_0.8.0_x64_en-US.msi", browser_download_url: `${repository}/releases/download/app-v0.8.0/Aster.Launcher_0.8.0_x64_en-US.msi` }];
  ...
  for (const invalid of [
    release("0.8.0", { assets: [] }), release("0.8.0", { draft: true }), release("0.8.0", { prerelease: true }),
    release("0.8.0", { assets: [{ name: "arm64-setup.exe", browser_download_url: `${repository}/releases/download/app-v0.8.0/arm64-setup.exe` }] }),
    release("0.8.0", { assets: [{ name: "Aster_0.8.0_x64-setup.exe", browser_download_url: "https://github.com/someone/another-project/releases/download/app-v0.8.0/setup.exe" }] }),
  ]) {
    latest = invalid;
  ...
  assert.ok((await (await site.fetch(new Request("https://aster.test/changelog"))).text()).includes("temporarily unavailable"));
  assert.equal((await site.fetch(new Request("https://aster.test/download"))).status, 503);
  requests.length = 0;
